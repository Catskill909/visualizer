...
| **Unconditional audio strip on video import** — every video that enters the app now passes through `stripAudio()` (lossless `ffmpeg -c:v copy -an` remux) before going to IndexedDB. Previously only oversized clips lost audio (via `-an` inside `transcodeTo720p`); ≤720p uploads kept their audio track, and importing a longer clip with audio could grab the MediaSession in WKWebView and break the main app's audio player. Strip runs at both upload (`_addVideoLayer`, non-transcoded branch) and import (`importPreset` in `customPresets.js`, dynamic-imported to keep FFmpeg out of the main bundle). All three video-element creation sites also set `volume = 0` alongside `muted = true` as a belt-and-suspenders guard, and the preset-reload path logs a `console.warn` if any stored video still reports `audioTracks.length > 0`. Invariant comment block at the top of `_addVideoLayer` documents the rule. | May 20 |
