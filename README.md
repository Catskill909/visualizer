...
- **Multi-display output** *(web app — beta, in active development)* — push the live visualizer to a second monitor or projector as a **pixel-perfect mirror**. In the player & editor, `O` → **Send to display** opens a synced output window; in the **Timeline**, the `⊟ Outputs` modal routes each zone to its own display so different presets drive different screens (compact `▸`/`▸N` chip per zone row). Built as a copy-the-pixels mirror (`canvas.captureStream()` → a `<video>` in the output window), so the second screen is always frame-identical to what you see — no drift, no second engine, no audio path. **Web only for now;** the native macOS/Windows output pipe (Syphon/Spout), saved routes, and stacking many presets onto one screen are upcoming phases — see [`output-dev.md`](output-dev.md).
...
| [`output-dev.md`](output-dev.md) | **Output & multi-monitor hub (truth doc).** Pixel-mirror architecture — the output window is a `<video>` copy of the source canvas, never a re-render (§13 post-mortem on the drift that killed the re-render approach). Source/Output/Route model; one swappable per-platform pixel pipe. **Status: web shipped — player/editor single mirror (A1) + timeline per-zone→display routing (A2); native Mac/Win pipe, route persistence, and stacking remain.** |
