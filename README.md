...
| [`output-dev.md`](output-dev.md) | **Output & multi-monitor hub (truth doc).** Pixel-mirror architecture — the output window is a `<video>` copy of the source canvas, never a re-render (§13 post-mortem on the drift that killed the re-render approach). Source/Output/Route model; one swappable per-platform pixel pipe. **Status: web shipped — single mirror (A1) + per-zone routing (A2) + stacking/layering (A3, layers follow the timeline). Shipping product is the desktop app; native Mac/Win pixel pipe (NDI-first) is the next track.** |
| [`native-output-dev.md`](native-output-dev.md) | **Native output pipe — NDI / Syphon / Spout (the native half of `output-dev.md`).** Deep design + plan: NDI-first (cross-platform, → OBS/streaming/other machines), the frame compositor, JS→native transport options + throughput math, and why video is *not* a taint blocker. 📋 Planning — read before any native output code. |
