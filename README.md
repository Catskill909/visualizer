...
| [`milkdrop-tools-dev.md`](milkdrop-tools-dev.md) | ✅ Phases 1–3 + Color v1 shipped 2026-05-30 | **MilkDrop from-scratch creation tools.** Raises the blank-canvas ceiling without a code/knob wall. **Motion Engine** (living time-driven motion via injected `frame_eqs`), **Custom Shapes Composer** (up to 4 placement-first shapes), **Shape Motion & Reactivity** (per-shape Spin/Orbit + full Source/Curve/Size/Opacity/Spin/Shake/Sides reactivity + Trail), and **Color Studio v1** (🎲 harmony-aware random palette). Phase 4 (Warp shader variants) deferred to its own session/doc; Phase 5 (expert eqn/shader drawer) optional. Phase tracker + per-phase execution notes at top. |
