...
| [`milkdrop-tools-dev.md`](milkdrop-tools-dev.md) | ✅ Phases 1–3 + Color Studio shipped 2026-05-31 | **MilkDrop from-scratch creation tools.** Raises the blank-canvas ceiling without a code/knob wall. **Motion Engine** (living time-driven motion via injected `frame_eqs`), **Custom Shapes Composer** (up to 4 placement-first shapes), **Shape Motion & Reactivity** (per-shape Spin/Orbit + full Source/Curve/Size/Opacity/Spin/Shake/Sides reactivity + Trail), and the **Color Studio** — 🎲 random + steerable harmony-rule / tone / Base-Hue generator + **Color Roll** (time-cycling hue) + **Glow/Accent bloom** (colored halo, replaces the border-ring sliders). Also a deep **trail/decay audit** (§9.13–9.14): default `decay` lowered across BLANK + all variations into a clean band so nothing ships with permanent gray trails. Phase 4 (Warp shader variants) and Phase 5 (expert eqn/shader drawer) **removed** — §11 (Phase 4 can regress all 1,144 presets; Phase 5 off-brand). Phase tracker + per-phase execution notes at top. |
