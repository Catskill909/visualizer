...
- **Motion & Wave** — six one-click Motion Presets (Vortex / Calm Drift / Earthquake / Tunnel In / Spin Lock / Hyperspace); a forked wave engine with a variable **Thickness slider (0–8)** and **Rotation slider (±180°)** that affect all 8 shape modes; **Wave Reactivity** (audio-driven Size / Opacity / Shape / Orbit, each with its own source pill) parallel to Motion reactivity; a Shape Reset that hides the wave until you actively pick a shape
- **Image / Video / GIF / Text layers** — up to 5 layers in a collapsible accordion stack with drag-to-reorder and per-layer solo / mute / rename / thumbnail:
  - *Video* — drag MP4/WebM (auto-transcodes 1080p/4K to 720p on upload), full playback controls (play/pause, loop, 0.25×–4× speed, scrub), 12-control color grading, independent Width/Height scaling, and a feathered Border ring
  ...
  - *Per-layer effects* — UV mirror, audio reactivity (Bass / Mid / Treble / Volume × Linear / Squared / Cubed / Gate), Saturation, Hue Rotate, and Aspect mode (Lock / Fluid)
- **Tiling & Grids** — aspect-correct density tiling for any image shape; an explicit **Grid mode** (Cols×Rows with Fit/Fill + Scale); **recursive grids** (Subdivide + Outer Gap); a **per-cell suite** — Offset (brick/half-drop), Cell Rotate (Truchet mosaics), Popcorn (per-cell audio pulse), Size/Jitter/Opacity Var with scatter sampling (jittered tiles break free of the grid and overlap), and Seed + Lock; plus **Pan** (Drift / Bounce). Works on videos too
- **VJ Effects** — Chromatic Aberration, Luma Key, Wave Distort, Invert, Solarize, Threshold, Pixelate, Scan Lines, and Film Grain — all per-layer
