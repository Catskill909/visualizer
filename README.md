...
A standalone visual preset builder. Four tabbed sections (Palette / Motion / Wave / Images), undo/redo (50-deep), A/B comparison, double-click any slider label to reset it, and an icon footer (**New** / **Random** / **Remix** / **Save** / **Reset**) — where **Random** loads any of the 1,144 bundled presets as an editing base (turning the Studio into 1,144 starting points), and **🎲 Remix** *(NEW)* rolls an **entire from-scratch preset in one press** — a moving colour-field background, three contrasting colours, a living motion + a warp flow, and content (the roll picks a *type*: a subtle wave ~20%, audio-reactive shapes/blobs ~55%, or a pure flowing field ~25%), all pulsing to the audio. **Lock a preset** *(NEW)* — in the collapsible **🔒 Locks** strip above the footer, **Lock preset** flips Random from *browse* to *vary*: it stops loading new presets and instead **re-moods the one you're on** (colour grade, grade reactivity, Scene FX, Club) while keeping its motion identical — endless variations of any of the 1,144, the same doom-scroll discovery but focused on a preset you chose (unlock to browse fresh presets again). The same Locks strip holds the **Remix · pin what to keep** chips (Colours / Field / Motion / Flow / **Reactivity**) so 🎲 Remix re-rolls only the rest — the dice as a compositional tool. Saves to localStorage; dev performance HUD on the `` ` `` key.

- **Palette & Color** — 12 one-click triad palettes (Wave / Glow / Accent) with hover-to-preview, per-channel lock, and a "+ Save current mix" button that pins the active triplet as a 13th "My Mix" chip; a **Color Studio** *(NEW)* — a **🎲 Colors** button rolls a full harmony-aware random scheme, and under it a *steerable* generator: **harmony-rule** chips (monochrome / analogous / complementary / split-complement / triadic), **tone** chips (Vivid / Neon / Pastel / Deep), and a **Base Hue** slider; pick any to steer the roll, leave them clear for pure random, click an active chip to clear it, and 🎲 lights up whatever it landed on. A **Color Roll** slider cycles the whole scheme's hue over time (works in solid *and* feedback modes). A **Color Field** *(NEW)* turns the background into a *living, moving multi-colour field* — it spreads the Shift A→B blend across the screen (**Flat** = the classic flat Shift, plus **21 moving styles** — Linear, Stripes, Weave, Radial, Diamond, Moiré, Conic, Spiral, Rays, Vortex, Mandala, Plasma, Clouds, Marble, Ripples, Checker, Hex, Chevron, Dots, Grid, Caustics — each with **Scale / Motion / Spin / Sharpness / Warp** sliders (Warp domain-warps the field for an organic ripple on *any* pattern), an optional **3-colour** blend, and a **beat-reactive** field that breathes to the music) while still pulsing to the audio, so the background is no longer a flat slab; a separate **Background** colour swatch keeps the field's colour *distinct from* the wave/shapes/flow drawn over it (the 🎲 roll sets them to contrasting harmony colours), fixing the "background and animation are the same colour" collapse. Honours the per-channel locks (lock a colour you like, reroll the rest); 3 independently overridable color swatches; **Opacity** (fades the MilkDrop background to black while layers render on top at full brightness); a **Show layers only + Transparent background** pair that makes the whole canvas truly transparent so layers float on nothing (the foundation for stacking presets in the Timeline); **Glow / Accent Strength** *(NEW)* — now a colored **bloom**, a soft halo tinted by the Glow / Accent colour so all three palette colours show on the visualisation itself (not the old edge-border ring; the literal Outer/Inner Border sliders remain for true borders); **Color adjustments** *(NEW)* — **Brightness / Contrast / Gamma / Temperature** faders (alongside Saturation / Hue Rotate / Color Roll) that adjust the *final output of any loaded preset* — including the 1,144 bundled MilkDrop presets — so you can load a preset and re-mood it live (warm/cool, punch contrast, lift mids); plus a **Color Reactivity** *(NEW)* panel — pick a **Source** (Bass / Mid / Treble / Volume / Flux) + **Curve**, then dial how hard each adjustment **pulses to the beat**, so any of the 1,144 presets can breathe with the music; a **Scene FX** *(NEW)* rack — **Bloom** (soft glow from the blurred image — softens/fills any preset incl. the 1,144 bundled), **Posterize / Vignette / Scan lines / Film grain** that finish the final image of *any* loaded preset; a **🌙 Club / Dark Room** *(NEW)* one-knob dark-room tune for *any* loaded preset — it detects **blown-out white** (the room-lighting enemy) and crushes *that* specifically while pushing the remaining colour **deeper, not dimmer**, so whites collapse toward dark but reds/blues/greens get richer; a one-tap **Club it** button snaps it on (the refined version of the Invert hack); Trail, Fade Wave in Silence, Outer/Inner Border sliders, and Brighten / Solarize / Darken / Darken Center / Invert toggles
- **Motion & Wave** — the Motion tab is one clean band: a **Motion** engine of living, time-driven motion (**Breathe / Sway / Spin**, each with **Speed + Depth** knobs) that makes a from-scratch preset evolve instead of sitting on frozen `baseVals` (injects per-frame `frame_eqs`, plays live in player & timeline); a **Flow** *(NEW)* per-preset **warp-shader library** — sharp fields (**Tunnel / Spiral / Ripple / Swirl / Plasma / Liquid / Kaleido**) plus **soft** blur-sampled fields (**Bloom / Smoke / Melt**) that fill as broad glow instead of thin threads — each with **Speed / Depth / Density** knobs (the *motion field* that gives 85% of bundled presets their signature look — now no-code, and it stacks *with* a Motion engine); **Quick Looks** (one-tap motion combos); and below them, every control (Movement, Echo, Drift, Warp Center, Randomize, and a full **Reactivity** panel) laid out in the open — audio reactivity stays front-and-centre, never hidden behind a drawer. Plus a forked wave engine with a variable **Thickness slider (0–8)**, a **Fill slider (0–1)** *(NEW)* that fills the wave into a broad shape — a pulsing disc on the round modes, a solid wedge on the line modes — instead of a thin oscilloscope thread, and a **Rotation slider (±180°)** across all 8 shape modes, and a Shape Reset that hides the wave until you actively pick a shape
- **Custom Shapes** *(NEW, Wave tab)* — place up to **4 geometric shapes** from scratch, each a placement-first card: drag an **XY-pad** to position, then **Size** (fills the frame), **Sides** (triangle → circle, curved slider), **Angle**, **Opacity**, **Fill** colour, **Border** on/off + colour, and **Glow** (additive). Per-shape **Motion** (Spin / Orbit, time-driven) and a full Wave-style **Reactivity** panel — **Source** + **Curve** + **Size / Opacity / Spin / Shake / Sides** amounts, each with its own per-slider source pill. A **Trail** slider controls how long shapes smear (true zero = crisp, up = long echoes). Shapes render over the feedback variations *and* over the flat Solid/Shift backgrounds. No code — pure engine `baseVals` + generated `frame_eqs`, so they play identically in the player & timeline
...
- **Image / Video / GIF / Text layers** — up to 5 layers in a collapsible accordion stack with drag-to-reorder and per-layer solo / mute / rename / thumbnail. On a bundled MilkDrop preset a layer **sits still by default** *(NEW)* — it no longer inherits the preset's motion; its own audio sliders are the opt-in for reactivity — Blend modes apply cleanly, and layers **persist across the Random button** (matching Remix), so you can browse presets under a fixed logo or clip without it vanishing:
...
| [`image-texture-dev.md`](image-texture-dev.md) | ✅ Shipped + cross-platform verified | **Meld (image-as-texture)** — feed an image/video/GIF layer INTO the preset's feedback loop so the preset *processes* it (melts/tunnels/kaleidoscopes, audio-reactive) instead of overlaying it. Per-card **Meld** button; one-knob melding tools — Size/Position/Luma-Key/**Mask** (crisp logo stencil)/Mirror/Colour-Grade/log-Speed/**Blend mode**/**Displacement** (image shape ripples the melt)/**Flow Map** (image edges steer the melt's motion along its contours)/**Palette Tint** (the melt wears the image's own dominant colours)/**Edge Feather** (softens a transparent-video cutout's matte so it stops sparkling in the melt — small/low-res cutouts now clean); 🎲 Remix rolls the whole melt. Pure WebGL (web/macOS/Windows). Plus the **🌙 Club / Dark Mode** final-output knob (§18). No engine fork (reuses `setUserTexture` + warp injection). |
| [`milkdrop-control-dev.md`](milkdrop-control-dev.md) | 📋 Planning (handoff-ready) | **MilkDrop Control & Remix convergence.** Make the 1,144 bundled presets first-class: bring Remix into sync with the Random button + expose curated control over a loaded preset (eventually Meld-into-any-preset). Design rule: **no slider-stacking; the dice is the discovery instrument.** Phased B-lite → A → B-full → C; **▶️ NEXT SESSION** = B-lite spike (roll the existing colour/reactivity/Scene-FX/Club axes on a loaded preset, zero new sliders). Meld-on-bundled is blocked with a friendly modal (`_bundledBase`). |
...
| [`video-cutout-edge-noise-dev.md`](video-cutout-edge-noise-dev.md) | ✅ SOLVED post-mortem — white edge speckle on melded transparent-video cutouts. Root cause: the web/native-alpha meld path ignored the matte entirely (alpha handling was macOS-stacked-only); unified the alpha gate + soft-feather **Edge Feather** matte blur. Lesson: a control that does *nothing* → check it's on the active code path first. |
