# MilkDrop Creation Tools — Archive

> Shipped execution detail + the original brainstorm/survey for the MilkDrop from-scratch
> creation tools. Pulled out of [milkdrop-tools-dev.md](milkdrop-tools-dev.md) on 2026-05-31 to keep
> the active planning doc lean. **This is reference / handoff history** — the live plan, phase tracker,
> and the next phase (Flow Style) live in `milkdrop-tools-dev.md`. Section numbers (§2–§11) are
> preserved from the original doc so cross-references still resolve.
