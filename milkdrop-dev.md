...
### Remix Family RX-1 — Inline-SVG icon footer (2026-05-20)
Replaced the five-pill footer (Remix / + New / ✨ Surprise / Save / Reset) with five equal-weight icon buttons. Order: New (plus) · Random (die) · Remix (shuffle) · Save (floppy) · Reset (rotate-ccw). Tooltips are single words ("New", "Random", "Remix", "Save", "Reset") positioned above each button via `tooltip-up` and explicitly centered via a `.panel-footer` selector that overrides the global `.editor-panel [data-tooltip]::after` left-align rule.

**Icons are inline SVG, not a webfont** — the first cut used Material Symbols Outlined via Google Fonts, but Tauri's WebView CSP (`style-src 'self' 'unsafe-inline'` at [src-tauri/tauri.conf.json:15](src-tauri/tauri.conf.json#L15)) blocks `fonts.googleapis.com`, so on the macOS and Windows builds the font stylesheet failed and the buttons rendered as literal text ("add", "casino", "shuffle", ...). Inline SVG (Lucide-style strokes, `stroke="currentColor"`) bypasses CSP entirely, works offline, renders identically on web/macOS/Windows, and is smaller than the font request. CSS rule `.footer-icon svg { width: 22px; height: 22px }` sizes them; the parent's `color:` drives the stroke. Lesson: anything that has to render in the Tauri WebView cannot rely on Google Fonts. Inter is also `<link>`-loaded but degrades acceptably via system-font fallback; icon fonts have no such fallback.
