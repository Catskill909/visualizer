...
| Phase | What | Date |
|-------|------|------|
| UI | Hide-panels toggle — new `#tl-btn-hideui` (left of the fullscreen button) hides the strip + transport + audio mini-player while **keeping the topbar**, for a windowed full-canvas view without entering true fullscreen. `toggleHideUI`/`_applyHideUI`; dynamic Hide/Show title; Esc restores; survives a fullscreen round-trip (`_showOverlays` re-asserts). Topbar action gap bumped 6→9px; Export/Import/Fullscreen tooltips shortened. **Gotcha fixed:** the eye/eye-off swap used an inline `style="display:none"` that beat the `.is-active` CSS, so the active button emptied out and collapsed its width (neighbours shifted, tiny click target). Both icons now toggle via the stylesheet only. | 2026-05-24 |
| Multi-track | **Add-track Phase A + B shipped.** Phase A (Full Screen, 2026-05-24): add/remove track, transparent gaps (empty upper track reveals beneath), top-row=front z-order, stacked tracks composite to the main/program output. Phase B (multi-region, 2026-05-26): Left/Right, quadrants, Center+Frame, Top Banner all support stacking — per-region `+ Track` button inline on each region's front-layer row (no extra strip rows), per-region output routing in the Output Manager, output chip anchored to each region's foundational zone so it's always visible at a glance. Also defensive: `mix-blend-mode` only written for non-`normal` modes so straight-alpha canvases composite via the default path. Full detail + roadmap in [`add-track-dev.md`](add-track-dev.md). | 2026-05-26 |
| bugfix | Overlay no longer blacks out when the zone beneath it has a gap — every zone's black gap-cover sat at a flat `zIndex: 10`, above *all* canvases, so a bottom zone's full-screen cover painted over any overlay zone stacked above it (e.g. Frame's block ends → Center overlay goes black). Covers now interleave with canvases: canvas at `zone.zIndex*2`, its cover at `*2+1`, so a gap-cover only hides its own zone. (`_positionCanvas`, `_syncZoneCanvases` cover block.) | 2026-05-24 |
| bugfix | Layout switch now blanks the stage — `_applyLayout` was clearing `entries` + the strip blocks, but `stop()` is VJ-mode (engines keep running, covers stay revealed) and the surviving `full` zone (= primary engine, present in every layout) kept its canvas painted with the prior preset and its cover at opacity 0. Result: a stuck preset with no block left to remove it. Fix: after `_syncZoneCanvases`, `_applyLayout` now `_currentZonePreset.clear()` + re-covers every zone (`cover.style.opacity = '1'`). | 2026-05-24 |
| bugfix | Stacked transparent layers sit on top instead of blending — the **Center + Frame** (Center) and **Top Banner** (Banner) layouts hardcoded `blendMode: 'screen'`, which lightens the top layer's colours into the layer below (looked fine loaded alone; "blended, not on top" when stacked). Both defaults changed to `'normal'`; `screen` is still opt-in per-zone via the stack blend control (`_buildStackControls`). | 2026-05-24 |
| — | Video play() AbortError no longer shows a red RENDER ERROR banner — during fast block transitions a video's `play()` promise gets superseded by the teardown `pause()`/`src=''`, rejecting with a benign `AbortError`. Both `play().catch` handlers in `visualizer.js` (`_loadVideoTexture` ~1335 and the `ended` loop-restart ~1345) now swallow `AbortError` (quiet `console.debug`) and only banner genuine failures (`NotAllowedError` autoplay-block, `NotSupportedError` codec). | 2026-05-21 |
| — | Preset picker lazy scroll — the Add Preset modal's "All" tab no longer caps at 100 with a dead "… N more — type to narrow" line. `_filterPicker` now renders the first 100-item chunk and a `scroll` listener on `#tl-picker-list` appends the next 100 as you near the bottom (`_renderPickerChunk` / `_makePickerItem`). All 1187 presets browsable without typing; search still narrows instantly. | 2026-05-19 |
...
> **⚠️ Update 2026-05-24 — reconcile with what shipped since this was written:**
> - **Per-zone opacity + blend-mode controls already exist** (`_buildStackControls`; `_positionCanvas` already sets `canvas.style.opacity`/`mixBlendMode` from `zone.opacity`/`zone.blendMode`). So most of **4.9-A is effectively built** — what's left is the *gap-behavior* control and a tidy popover, not opacity/blend wiring.
> - **The `screen`-blend "black passthrough" trick below is now the *legacy* path.** Transparent-bg presets (true per-pixel alpha) shipped 2026-05-24 — that's the cleaner reveal mechanism (a transparent-bg preset on a `normal`-blend zone sits on top and reveals the zone beneath via real alpha). The fixed layouts that used to default to `screen` (Center+Frame, Top Banner) were **changed to `normal`** so stacked layers sit on top, not lighten-blend. `screen` is still selectable per-zone for the old palette-opacity workflow.
> - **4.9-B (Overlay layout)** was reconsidered and **parked** — see [`add-track-dev.md`](add-track-dev.md). A full-screen overlay with transparent gaps belongs to add-tracks (one `gapBehavior:'transparent'` flag), not a one-off `screen`-blend layout tile.
