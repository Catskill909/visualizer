...
`2026-05-25` — **Aspect mode (Lock / Fluid) added — per-layer; affects all tiling paths.** New `entry.aspectMode` ('lock' default) gates the `aspectPreScale` factor: **Lock** = `imgAsp * (aspect.y / max(aspect.x, 0.01))` (true shape on any canvas orientation — tiles stay one shape, resize reveals more/fewer), **Fluid** = legacy `imgAsp * aspect.y` (correct in landscape, squishes in portrait). `aspect.x = 1` in landscape so the modes are identical there — they only diverge on a portrait canvas. Default applied in `_normalizeImageEntry`; segmented Lock/Fluid toggle on the layer card (mirrors the Fit/Fill pattern), non-grid only. Covers single + plain-tiled + tunnel + scatter (everything that runs `aspectPreScale`). **Phase 3 (grid mode) shipped same day:** the three grid aspect sites (`cellAspectExpr` + both Fit `_cellAR`) now use `aspect.y / max(aspect.x, 0.01)` too, so grid Fit and per-cell rotation are portrait-correct; the inert Lock/Fluid toggle is hidden in grid (Fit/Fill is grid's shape control, gated via `syncGridVisibility`). Also fixed same day: editor `sizeCanvas()` was CSS-stretching the whole canvas in full-screen/focus mode (drawing-buffer-must-match-display rule — see `aspect-ratio.md` §9). Aspect-ratio work is now complete end-to-end. Full spec/rationale/verification: **`aspect-ratio.md`**.
