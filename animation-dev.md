...
**P0 ✅ + A1 Gate 1 ✅ + A3 ✅ shipped & user-verified 2026-05-27. A2 🔧 wired same day, awaiting verification.** Entrance + Idle live; Exit just wired: chip row + duration + ease (`.in` variants — Smooth / Snappy / Linger / Wobble / Linear), Preview button is now tab-aware (entrance / exit) and hidden on Idle. Layer delete awaits `playExitAnimation` before splicing; card freezes visually for the tween's duration.

**Remaining:** A1 Gate 3 (custom visual scrubber + bezier editor). A4 (UI polish) is last.
...
| 3 | **A2** — Exit tab | Mirror of entrance; layer waits for exit tween before hiding | 🔧 wired (unverified) |
