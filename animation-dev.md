...
**⏱ A6 — per-layer Enter at / Exit at TIMES ✅ shipped 2026-05-28 (verify in player/timeline).** Each layer can now be scheduled to **enter** and **exit** at a chosen time (0:00–3:00) after the preset loads — a per-layer mini-timeline inside the preset. This is the *timing* (when), distinct from the entrance/exit *Duration* (how long the motion takes — unchanged). Two new `m:ss` time controls in the Animate modal ("Enter at" on Entrance, "Exit at" on Exit). The schedule plays during real playback in the **player & timeline** (built on A5); the **editor stays a workbench** (layers load visible so you can tune them). Default `entranceAt:0 / exitAt:0` = enter immediately / no scheduled exit → pre-A6 presets behave exactly as before. See "A6 — Enter at / Exit at times" in Recently shipped.
...
| 5″ | **A6** — Enter at / Exit at times | Per-layer schedule (0:00–3:00) for when each layer enters & exits; plays in player & timeline | ✅ shipped 2026-05-28 (verify in player/timeline) |
...
- **2026-05-28 — A6 Enter at / Exit at times ✅ shipped (verify in player/timeline).** Per-layer schedule inside a preset: each layer can enter and exit at a chosen time, 0:00–3:00 after the preset loads. This is the *timing* the original request asked for ("entrance and exit **times**") — **not** the entrance/exit Duration (how long the fade/slide takes), which is untouched. Took two wrong turns first (extending the Duration slider) before locking the mandate: it's *when* the layer enters/leaves, not how long the animation runs.
  - **Data model.** `entranceAt` + `exitAt` (seconds, 0–180) added to `DEFAULT_ANIMATION` ([inspector.js](src/editor/inspector.js)). `0` = enter immediately / no scheduled exit, so every pre-A6 preset is byte-identical. Persisted automatically (`saveCurrent` spreads `currentState.images`; `_normalizeImageEntry` backfills defaults on load).
  - **Scheduler.** New `scheduleLayerAnimations(entry, cfg, refresh)` + `killLayerSchedule(entry)` in [animation.js](src/editor/animation.js). Uses `gsap.delayedCall` as the clock — it starts when called (= preset load), so the schedule resets to zero on every load (matches "start from zero when loaded"). Before its entrance time a layer is held at `opacity 0` via the q-pipe (invisible); at `entranceAt` the entrance plays (or it just appears if entrance = none); at `exitAt` the exit plays (or hard-hides if exit = none). Idle starts after the entrance settles. Handles stored on a non-enumerable `entry._schedule` (won't trip `JSON.stringify`).
  - **Where it runs.** The **engine** drives the schedule, so it plays in the **player & timeline** (the A5 path): `_startPresetAnimations` now calls `scheduleLayerAnimations` per layer; `_killPresetAnimations` calls `killLayerSchedule` + kills tweens on every swap (no leaked `delayedCall`s). The **editor's `loadPresetData` is deliberately left alone** — the editor is a workbench, layers load visible so you can tune them. You set the times in the editor modal; you watch them play in the player/timeline (where A5 already works).
  - **UI.** Two new `m:ss` time scrubbers — "Enter at" (Entrance tab) + "Exit at" (Exit tab). `_hydrateScrubber` gained a `data-format="time"` mode: `m:ss` readout, magnitude-aware snapping (1s under a minute, 5s above), and labeled landmark ticks (`0:00 · 0:30 · 1:00 · 2:00 · 3:00`) that read like a timeline ruler. Linear scale (range starts at 0, so no log needed). The `'s'` (Duration) and `'x'` (Idle speed) scrubbers are byte-identical to before — the new behaviour is gated on `fmt === 'time'`, and the labels row collapses (`:empty { display:none }`) so those scrubbers don't change layout.
  - **Not done / to verify:** confirm in a real **player & timeline** session — set e.g. *Enter at 0:00, Exit at 0:05* on a layer (short times so it's quick to watch), save, load in the player, confirm the layer appears then leaves on cue; repeat in the timeline. Editor-side preview of the schedule is intentionally NOT included (workbench) — revisit only if you want to scrub the schedule in the editor too. Sub-second entrance staggers aren't supported (whole-second snapping under a minute) — by design for clean `m:ss`; revisit if cascade staggers need 0.5s.
...
- **2026-05-27 — A1 Gate 3 ✅ shipped & user-verified.** Two reusable custom controls replace the native form widgets in the Animate modal's Duration / Ease rows:
  ...
  - Wiring: handles stored on `this._animateScrubEntrance / _animateEaseEntrance / _animateScrubExit / _animateEaseExit / _animateScrubIdleSpeed`. `_syncAnimateModal` calls `setValue` / `setActive` on the stored handles — no per-tick `getElementById`. Old `animate-duration-val`, `animate-exit-duration-val`, `animate-idle-speed-val` `<span>` readouts deleted along with their parent rows.
  - Gate-2 contextual sliders (Distance / Start size / Pop from / Start blur) stay as `<input type=range>` for now — they're per-preset params, not duration / ease, so they're outside Gate 3's scope. A later sweep could convert them to scrubbers for visual consistency.
