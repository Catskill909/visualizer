...
- [~] **N0b — Real-frame transport + measurement (in progress).**
  ...
  - [!] **PARKED FROM THE BUILD 2026-05-24.** Bundling `ndi-send` broke `./build-and-sign.sh` Step 8 (DMG): the sidecar's external `@rpath/libndi.dylib` made Gatekeeper hold the mounted DMG volume → the post-AppleScript `hdiutil detach` went busy → build aborted before the DMG reached `promo/`. Since NDI can't ship until N2 anyway (libndi unbundled), `ndi-send` was removed from `externalBin` + `shell.scope` (tauri.macos.conf.json, back to ffmpeg-only) and the NDI button is gated behind `NDI_ENABLED=false` (timelineEditor.js). The `ndi-send` binary, Rust commands, and `ndi-spike/` all stay committed. **Re-enable at N2:** bundle `libndi` into the app + fix the sidecar rpath (→ `@executable_path`), then restore `externalBin`/scope and flip `NDI_ENABLED`. (commit `fix(build): unbundle ndi-send…`.)
