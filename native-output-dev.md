...
- [~] **N0b — Real-frame transport + measurement (in progress).**
  ...
  - [ ] **App wiring (needs `tauri-dev`):** chosen architecture = **Rust-relay sidecar** (confident on Tauri v1.5; keeps `libndi` out of the main app). Register `ndi-send` like ffmpeg (`tauri.macos.conf.json` → `allowlist.shell.scope` + `bundle.externalBin`); Rust `ndi_start`/`ndi_send_frame(b64)`/`ndi_stop` spawn the sidecar (`new_sidecar`) and `CommandChild::write` length-prefixed frames to its stdin; JS pumps `composer.canvas`→`toBlob('image/jpeg')`→base64→`invoke` at ~30fps behind an "NDI" toggle in the Outputs modal (Tauri-only).
  - [ ] **Measure** end-to-end in `tauri-dev` (OBS/Monitor), push toward 1080p; confirm video-composite readback in a production build.
