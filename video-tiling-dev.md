...
**Windows — no special handling needed (expected, smoke-test pending).** The stacked-alpha conversion is `isMacTauri`-gated at all three import sites (`inspector.js` ~1996 / ~2028 / ~2054: `!!window.__TAURI__ && navigator.userAgent.includes('Mac')`), so it never fires on Windows. Windows runs **WebView2 (Chromium)** — the same engine as the web build — so a transparent WebM decodes to an ordinary native-alpha RGBA texture and tiles via the **Phase A** path. The whole tile feature (un-gating, `sampleGrad`, UI) is platform-agnostic JS/GLSL — one file, runs identically everywhere. The macOS-only problem (WKWebView dropping VP9 alpha) does not exist in WebView2. A Windows run would *confirm* the native-alpha path behaves like web, not exercise a new code path.
**Verified:** Phase A in browser (opaque + native-alpha transparent). Phase B in a packaged macOS build — transparent video tiles with all tiling sliders and the grouping (Group spin) functions working. Windows: pending a 30-second native-alpha smoke test (drop transparent WebM → Tile → Density + Grid).
